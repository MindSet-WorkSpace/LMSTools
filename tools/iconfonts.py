...
def create_fontdict_file(css_fname, output_fname):
    ...
    with open(css_fname, 'r') as f:
        data = f.read()
        res = _parse(data) if css_fname.endswith(".css") else __parse(data)
        with open(output_fname, 'w') as o:
            o.write(json.dumps(res, indent=4))
        return res
...
def __parse(data):
    ...
    for datas in new_data:
        if datas == "":
            continue
        key, value = datas.split(" ")
        res.update({key: int(f"0x{value}", 0)})
    return res


if __name__ == "__main__":
    create_fontdict_file(sys.argv[1], sys.argv[2])
